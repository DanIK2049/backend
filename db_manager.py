...
def device_exists(mac_address):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM known_devices WHERE mac_address = ?", (mac_address,))
    (count,) = c.fetchone()
    conn.close()
    return count > 0
...
def insert_metrics(device_id, rx, tx, conns):

    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""
        INSERT INTO device_metrics (device_id, rx_bytes, tx_bytes, connections)
        VALUES (?, ?, ?, ?)
    """, (device_id, rx, tx, conns))
    conn.commit()
    conn.close()
...
def get_all_metrics():
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT rx_bytes, tx_bytes, connections FROM device_metrics")
    rows = c.fetchall()
    conn.close()
    return rows
